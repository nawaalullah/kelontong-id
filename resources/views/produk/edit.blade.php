...
@section('content')
    <div class="max-w-2xl">
        <div class="rounded-xl bg-white border border-ink-100 shadow-paper p-6">
            <form action="{{ route('produk.update', $produk) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="kode_produk" value="{{ old('kode_produk', $produk->kode_produk) }}" required
                           class="w-full rounded-lg border border-ink-100 bg-paper/40 px-3.5 py-2.5 text-sm font-mono text-ink-700 focus:outline-none focus:ring-2 focus:ring-mustard-400 focus:border-mustard-400">
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_kadaluarsa"
                           value="{{ old('tanggal_kadaluarsa', $produk->tanggal_kadaluarsa?->format('Y-m-d')) }}"
                           class="w-full sm:w-1/2 rounded-lg border border-ink-100 bg-paper/40 px-3.5 py-2.5 text-sm text-ink-700 focus:outline-none focus:ring-2 focus:ring-mustard-400 focus:border-mustard-400">
