            ...
            <form action="{{ route('kategori.update', $kategori) }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="3"
                              class="w-full rounded-lg border border-ink-100 bg-paper/40 px-3.5 py-2.5 text-sm text-ink-700 focus:outline-none focus:ring-2 focus:ring-mustard-400 focus:border-mustard-400 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">{{ old('deskripsi', $kategori->deskripsi) }}</textarea>
                </div>
